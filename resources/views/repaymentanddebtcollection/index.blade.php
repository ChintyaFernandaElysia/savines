...
@section('title', 'Repayment / Debt Collection')
...
<div class="card shadow mb-4">
    ...
    <div class="mx-3 mt-3">
        <table class="table table-bordered" style="width:100%">
            ...
            @php($no = 1)
            @foreach ($repaymentanddebtcollection as $data)
            <tr>
                <td>{{ $no++ }}</td>
                <td>{{ $data->date }}</td>
                <td>{{ $data->title }}</td>
...
{!! $repaymentanddebtcollection->links() !!}
    ...
    <script>
        ...
        function read() {
            ...
            $.get("{{url('repaymentanddebtcollection/read')}}", {}, function(data, status){
                $("#read").html(data);
            });
        ...
        function create() {
            ...
            $.get("{{url('repaymentanddebtcollection/create')}}", {}, function(data, status){
        ...
        function show(id) {
            ...
            $.get("{{url('repaymentanddebtcollection')}}/" + id, {}, function(data, status){
